...
void rgblight_effect_breathing(animation_status_t *anim) {
        ...
        switch(i){
        case 0:if (dir == 0) {
            if (r < (rgblight_config.val-1)) r+=2;
            else r++;
            }
                if(r == rgblight_config.val) dir = 1;
                if (dir == 1) {
                        if (r>1) r-=2;
                else r--;
                        if(r == 0) {dir = 0;i=1;}
        ...
        case 1:if (dir == 0){
                if(g <(rgblight_config.val-1)) g+=2;
            else g++;
            }
                if(g == rgblight_config.val) dir = 1;
                if (dir == 1) {
                        if (g>1) g-=2;
                        else g--;
                        if(g == 0) {dir = 0;i=2;}
        ...
        case 2:if (dir == 0){
                if (b < (rgblight_config.val-1)) b+=2;
                    else b++;
            }
                if(b == rgblight_config.val) dir = 1;
                if (dir == 1) {
                        if (b>1) b-=2;
                        else b--;
        ...
        case 3:if (dir == 0) {
            if (r <(rgblight_config.val-1))  {r+=2;b+=2;}
            else {r++;b++;}
            }
                if(r == rgblight_config.val) dir = 1;
                if (dir == 1) {
        ...
        case 4:if (dir == 0) {
            if (r < (rgblight_config.val-1)){ r+=2;g+=2;}
            else {r++;g++;}
            }
                if(r == rgblight_config.val) dir = 1;
                if (dir == 1) {
        ...
        case 5:if (dir == 0) {
                if(g < (rgblight_config.val-1)) {g+=2;b+=2;}
                else{g++;b++;}
            }
                if(g == rgblight_config.val) dir = 1;
                if (dir == 1) {
        ...
        case 6:if (dir == 0) {
                if (g <(rgblight_config.val-1)) {g+=2;b+=2;r+=2;}
                else {g++;r++;b++;}
            }
                if(g == rgblight_config.val) dir = 1;
                if (dir == 1) {
...
void rgblight_effect_breathing(animation_status_t *anim) {
    ...
    static uint8_t now_color = 0;
    if (!val) now_color = (now_color + 1) % 7;
    rgblight_sethsv_noeeprom_old(rgb_hsvs[now_color][0], rgb_hsvs[now_color][1], val);
    anim->pos = (anim->pos + 1);
}
#endif
#endif
...
#define my_val (rgblight_config.val-40)
void rgblight_effect_rainbow_mood(animation_status_t *anim) {
    static uint8_t i = 0,r = 0,g = 0,b = 0;
    r= my_val;
    if (rgblight_config.enable) {
        for(uint8_t j = 0;j< RGBLED_NUM;j++)
            rgblight_setrgb_at(r,g,b,j);

        if (i == 0) {if (g<(my_val-1)){r-=2;g+=2;} else {r--;g++;} if (g == my_val) i = 1;}
        if (i == 1) {if (b<(my_val-1)){g-=2;b+=2;} else {g--;b++;} if (b == my_val) i = 2;}
        if (i == 2) {if (r<(my_val-1)){b-=2;r+=2;} else {b--;r++;} if (r == my_val) i = 0;}
...
#ifdef rk773_light_mode1
void rgblight_effect_rainbow_swirl(animation_status_t *anim) {  //rk773专用
    uint8_t hue;
    static uint8_t i,j;
...
void rgblight_effect_rainbow_swirl(animation_status_t *anim) {  //rk773专用
    ...
    for (j = 0; j < 10; j++) {
        // 计算色调，与原始函数类似
        hue = (RGBLIGHT_RAINBOW_SWIRL_RANGE / 10 * j + anim->current_hue);

        // 设置当前LED的颜色
        sethsv(hue, rgblight_config.sat, rgblight_config.val, (LED_TYPE *)&led[j + rgblight_ranges.effect_start_pos]);

        // 设置镜像LED的颜色（对应于数组的另一半）
        sethsv(hue, rgblight_config.sat, rgblight_config.val, (LED_TYPE *)&led[(17-j) + rgblight_ranges.effect_start_pos]);
    }
    ...
    // for (j = 0; j < 2; j++) {
    //     hue = (RGBLIGHT_RAINBOW_SWIRL_RANGE / rgblight_ranges.effect_num_leds * j + anim->current_hue);
    //     sethsv(hue, rgblight_config.sat, rgblight_config.val, (LED_TYPE *)&led[j + rgblight_ranges.effect_start_pos]);
    // }

    // for (i = 2; i < 11; i++) {
    //     // 计算色调，与原始函数类似
    //     hue = (RGBLIGHT_RAINBOW_SWIRL_RANGE / 2 * i + anim->current_hue);

    //     // 设置当前LED的颜色
    //     sethsv(hue, rgblight_config.sat, rgblight_config.val, (LED_TYPE *)&led[i + rgblight_ranges.effect_start_pos]);

    //     // 设置镜像LED的颜色（对应于数组的另一半）
    //     sethsv(hue, rgblight_config.sat, rgblight_config.val, (LED_TYPE *)&led[(21-i) + rgblight_ranges.effect_start_pos]);
    // }
...
void rgblight_effect_rainbow_swirl(animation_status_t *anim) {
    uint8_t hue;
    uint8_t i;
    uint8_t half_num_leds = rgblight_ranges.effect_num_leds / 2; // 假设总LED数为偶数

    for (i = 0; i < half_num_leds; i++) {
        // 计算色调，与原始函数类似
        hue = (RGBLIGHT_RAINBOW_SWIRL_RANGE / half_num_leds * i + anim->current_hue);

        // 设置当前LED的颜色
        sethsv(hue, rgblight_config.sat, rgblight_config.val, (LED_TYPE *)&led[i + rgblight_ranges.effect_start_pos]);

        // 设置镜像LED的颜色（对应于数组的另一半）
        // sethsv(hue, rgblight_config.sat, rgblight_config.val, (LED_TYPE *)&led[i + half_num_leds + rgblight_ranges.effect_start_pos]);
